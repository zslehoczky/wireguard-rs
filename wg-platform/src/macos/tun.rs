...
use wg_traits::tun::{PlatformTun, Reader, Status, Tun, Writer};
...
impl MacosTunStatus {
    pub fn event(&mut self) -> Result<wg_traits::tun::TunEvent, StatusError> {
        ...
        loop {
            ...
            match if_msg {
                ...
                msg if msg.ifm_flags & libc::IFF_UP == 0 => {
                    break Ok(wg_traits::tun::TunEvent::Down);
                }
                msg => break Ok(wg_traits::tun::TunEvent::Up(msg.ifm_data.ifi_mtu as usize)),
            }
        }
    }
}
...
impl MacosTunWriter {
    pub fn write(&self, src: &[u8]) -> Result<(), io::Error> {
...
impl MacosTunReader {
    pub fn read(&self, buf: &mut [u8], offset: usize) -> Result<usize, io::Error> {
        let bytes_read = self
            .tun
...
impl MacosTun {
    pub fn create(
        name: &str,
    ) -> Result<(Vec<MacosTunReader>, MacosTunWriter, MacosTunStatus), MacosTunError> {
...
impl Status for MacosTunStatus {
    ...
    fn event(&mut self) -> Result<wg_traits::tun::TunEvent, Self::Error> {
        self.event()
    }
}
