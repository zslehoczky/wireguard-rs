...
pub struct LinuxTunStatus {
    events: Vec<wg_traits::tun::TunEvent>,
    index: i32,
    name: [u8; libc::IFNAMSIZ],
    fd: RawFd,
}
...
impl LinuxTunReader {
    pub fn read(&self, buf: &mut [u8], offset: usize) -> Result<usize, LinuxTunError> {
        /*
        debug_assert!(
            offset < buf.len(),
...
impl LinuxTunWriter {
    pub fn write(&self, src: &[u8]) -> Result<(), LinuxTunError> {
...
impl LinuxTunStatus {
    pub fn event(&mut self) -> Result<wg_traits::tun::TunEvent, LinuxTunError> {
                    ...
                    libc::RTM_NEWLINK => {
                        ...
                        if info.ifi_index == self.index {
                            ...
                            if info.ifi_flags & (libc::IFF_UP as u32) != 0 {
                                ...
                                self.events.push(wg_traits::tun::TunEvent::Up(mtu));
                            } else {
                                log::trace!("netlink, down event");
                                self.events.push(wg_traits::tun::TunEvent::Down);
                            }
                        }
                    }
                    _ => (),
