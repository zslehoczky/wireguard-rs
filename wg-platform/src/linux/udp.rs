...
impl LinuxUDPReader {
    pub fn read(&self, buf: &mut [u8]) -> Result<(usize, LinuxEndpoint), io::Error> {
        match self {
...
impl LinuxUDPWriter {
    pub fn write(&self, buf: &[u8], dst: &mut LinuxEndpoint) -> Result<(), io::Error> {
        match dst {
...
impl LinuxUDP {
    ...
    pub fn bind(
        mut port: u16,
    ) -> Result<(Vec<LinuxUDPReader>, LinuxUDPWriter, LinuxOwner), io::Error> {
        ...
        // create readers
        let mut readers: Vec<LinuxUDPReader> = Vec::with_capacity(2);
