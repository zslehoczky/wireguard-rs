...
impl<O> Device<O> {
    ...
    pub fn begin<R: RngCore + CryptoRng>(
        ...
        match (self.key_st.as_ref(), self.pk_map.get(pk.as_bytes())) {
            ...
            (Some(keyst), Some(peer)) => {
                ...
                let noise = noise::create_initiation(rng, keyst, peer, pk, local)?;

                // add macs to initation
                let macs = peer.macs.lock().generate(time, noise.as_bytes());
        ...
        match LittleEndian::read_u32(msg) {
            TYPE_INITIATION => {
                ...
                if let Some(src) = src {
                    ...
                    if !keyst
                        .macs
                        .check_mac2(time, msg.noise.as_bytes(), &src, &msg.macs)
                    {
                ...
                let (noise, keys) = noise::create_response(rng, time, peer, &pk, local, st)
                    .inspect_err(|_e| {
                        self.release(local);
                    })?;
            ...
            TYPE_COOKIE_REPLY => {
                ...
                peer.macs.lock().process(time, &msg)?;
    ...
    pub(super) fn lookup_pk(&self, pk: &[u8; 32]) -> Result<&Peer<O>, HandshakeError> {
